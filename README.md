...
```bash
dk bash  # starts bash inside "tartaruga" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and tartaruga.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `tartaruga` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The tartaruga container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv tartaruga -p python3  # creates a python3 virtualenv
